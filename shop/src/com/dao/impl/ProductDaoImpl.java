...
public class ProductDaoImpl implements ProductDao {
	...
	public void update(Product product) {
		...
		try{
			...
			pstmt.setString(5, product.getFilename());
			pstmt.setString(6, product.getPath());
			pstmt.setObject(7, product.getCategory().getCid());//setInt方法没有空值，因此此处设置为setObject
	...
	public int findCount() {
		...
		try{
			...
			conn = JDBCUtils.getConnection();
			// 编写SQL:
			String sql = "select count(*) as count from product";//此处设定查询出来的变量为count
			// 预编译SQL:
			pstmt = conn.prepareStatement(sql);
			// 执行SQL:
			rs = pstmt.executeQuery();
			if(rs.next()){
				count = rs.getLong("count");//此处查询出来为long型
			}
		}catch(Exception e){
			e.printStackTrace();
		}finally{
